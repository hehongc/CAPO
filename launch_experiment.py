...
from rlkit.torch.networks import Proto
...
def experiment(variant, seed=None):
    ...
    num_protos = 128
    proto = Proto(
        proj_dim=context_encoder_output_dim,
        pred_dim=512,
        T=0.1,
    ...
    repr_dim = 64
    encoder_sa = FlattenMlp(
        hidden_sizes=[net_size, net_size],
        input_size=obs_dim + action_dim + latent_dim,
        output_size=repr_dim,
    )

    encoder_rs = FlattenMlp(
        hidden_sizes=[net_size, net_size],
    ...
    generator = FlattenMlp(
        hidden_sizes=[200, 200, 200],
        input_size=obs_dim + action_dim + latent_dim + generator_dim,
        output_size=1 + obs_dim,
    )

    discriminator = FlattenMlp(
        hidden_sizes=[net_size, net_size],
        input_size=obs_dim + action_dim + 1 + obs_dim + latent_dim,
        output_size=1,
        ...
        if 'interpolation' in variant.keys() and variant['interpolation']:
            ...
            if 'goal_radius' in variant['env_params']:
                algorithm = CSROSoftActorCritic(
                    ...
                    nets=[agent, qf1, qf2, vf, c, encoder_sa, encoder_rs, generator, discriminator, club_model, proto],
                    latent_dim=latent_dim,
