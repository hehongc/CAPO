...
class SimpleReplayBuffer(ReplayBuffer):
    ...
    def random_batch_with_next(self, batch_size):
        ''' batch of unordered transitions '''
        assert self._size > 0
        indices = np.random.randint(0, self._size, batch_size)
        next_indices = indices + 1
        next_indices = np.clip(next_indices, 0, self._size - 1)
        return self.sample_data(indices), self.sample_data(next_indices)
